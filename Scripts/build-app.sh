...
# 0.1.0 bundle in ~/Applications has no SUFeedURL key at all. Sparkle with no
...
# the app: the bundle on disk is new, the process in front of the operator is
# the old binary, and every file along the way looks correct. Ten sibling
# repositories already end their install here; this one did not, so a fix
# shipped by `build-app.sh` reached Singularity only after the operator
# happened to quit it. `--if-running` leaves a stopped app stopped, and the
# helper launches with `open -g`, so nothing steals the foreground.
